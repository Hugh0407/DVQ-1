...
//		alertDialog.cancel();
    ...
    public static BarcodeDetailInfo BarcodeSplit(String sBarcode) throws Exception
    {
        if (sBarcode.equals(""))
            throw new Exception("该条码不符合规则");

        String[] lsSplitArray = sBarcode.split("|");
        if (lsSplitArray.length < 2)
            throw new Exception("该条码不符合规则");

        BarcodeDetailInfo lcBarcodeDetailInfo = new BarcodeDetailInfo();
        String lsBarcodeType = lsSplitArray[0];
        lcBarcodeDetailInfo.SetBarcodeType(lsBarcodeType);
        lcBarcodeDetailInfo.SetInventoryCode(lsSplitArray[1]);

        switch (BarcodeType.getBarcodeType(lsBarcodeType)) {
            case Y:
                if(lsSplitArray.length != 2)
                    throw new Exception("该条码不符合规则");
                break;
            case C:
                if(lsSplitArray.length != 6)
                    throw new Exception("该条码不符合规则");
                lcBarcodeDetailInfo.SetLotCode(lsSplitArray[2]);
                lcBarcodeDetailInfo.SetTaxFlag(lsSplitArray[3]);
                lcBarcodeDetailInfo.SetQuantity(Float.parseFloat(lsSplitArray[4]));
                lcBarcodeDetailInfo.SetSeriNo(lsSplitArray[5]);
                break;
            case TC:
                ...
                lcBarcodeDetailInfo.SetTaxFlag(lsSplitArray[3]);
                lcBarcodeDetailInfo.SetQuantity(Float.parseFloat(lsSplitArray[4]));
                lcBarcodeDetailInfo.SetNumber(Integer.parseInt(lsSplitArray[5]));
                lcBarcodeDetailInfo.SetSeriNo(lsSplitArray[6]);
                break;
            case P:
                if(lsSplitArray.length != 9)
                    throw new Exception("该条码不符合规则");
                lcBarcodeDetailInfo.SetLotCode(lsSplitArray[2]);
                lcBarcodeDetailInfo.SetOutsourcing(lsSplitArray[3]);
                lcBarcodeDetailInfo.SetTaxFlag(lsSplitArray[4]);
                lcBarcodeDetailInfo.SetQuantity(Float.parseFloat(lsSplitArray[5]));
                lcBarcodeDetailInfo.SetCWFlag(lsSplitArray[6]);
                lcBarcodeDetailInfo.SetOnlyFlag(lsSplitArray[7]);
                lcBarcodeDetailInfo.SetSeriNo(lsSplitArray[8]);
                break;
            case TP:
                ...
                lcBarcodeDetailInfo.SetTaxFlag(lsSplitArray[4]);
                lcBarcodeDetailInfo.SetQuantity(Float.parseFloat(lsSplitArray[5]));
                lcBarcodeDetailInfo.SetNumber(Integer.parseInt(lsSplitArray[6]));
                lcBarcodeDetailInfo.SetCWFlag(lsSplitArray[7]);
                lcBarcodeDetailInfo.SetOnlyFlag(lsSplitArray[8]);
                lcBarcodeDetailInfo.SetSeriNo(lsSplitArray[9]);
                break;
            default:
                throw new Exception("该条码不符合规则");
        }
        return lcBarcodeDetailInfo;

    }
    private enum BarcodeType {
                Y,C,TC,P,TP;

        private static BarcodeType getBarcodeType(String sType){
